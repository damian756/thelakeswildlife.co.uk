...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: species.faq?.[0]?.answer
      ...
      : `${species.commonName} (${species.scientificName}) on the Sefton Coast — where to find it, identification tips, and seasonal presence. ${species.seasonalPresence}.`,
  };
}
...
export default async function PlantPage({ params }: Props) {
  ...
  return <SpeciesDetail category="plants" species={species} related={related} wikiImage={wikiImage} />;
}
